...
<div class="card">
	...
	<div class="card-body">
		<a href="<?= base_url('surat_keluar/create'); ?>" class="btn btn-primary btn-sm btn-icon-split mb-3">
			<span class="icon">
...
<script>
	$(document).ready(function() {
		...
		var table = $('#tDokumen').on( 'draw.dt', function () {
			...
			'order': [],
			'ajax': {
				'url': '<?= base_url('surat_keluar/listData');?>',
				'type': 'post'
			},
			'columnDefs': [
	...
	function disposisi_keluar(id)
	{
		$.ajax({
			url: '<?= base_url('surat_keluar/disposisi'); ?>',
			type: 'post',
			data: {id : id},
			dataType: 'json',
			success: function(response) {
