		...
		<div class="card-body">
			<div class="mx-auto">
				<div class="alert alert-info" role="alert">
					...
					<div class="text-left">
						Jumlah Data Pengguna : <?= $jmlUser; ?>
					</div>
				</div>
			</div>
			<?= form_open_multipart('user/update', ['class' => 'formUpdate'], ['id' => $user['id_user'], 'fotoLama' => $user['foto'], 'created_date' => $user['created_date']]); ?>
			<?= csrf_field(); ?>
			<div class="row">
				<div class="col-md-9">
					<div class="form-group row">
						...
						<div class="col">
							<div class="input-group">
								...
								<input type="text" name="nama_lengkap" id="nama_lengkap" class="form-control <?= $validation->hasError('nama_lengkap') ? 'is-invalid' : ''; ?>" style="height: 38px" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.slice(1)" value="<?= ucfirst($user['nama_lengkap']); ?>">
					...
					<div class="form-group row">
						...
						<div class="col">
							<div class="input-group">
								...
								<input type="text" name="email" id="email" class="form-control <?= $validation->hasError('email') ? 'is-invalid' : ''; ?>" style="height: 38px" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.slice(1)" value="<?= ucfirst($user['email']); ?>">
					...
					<div class="form-group row">
						...
						<div class="col">
							<div class="input-group">
								...
								<input type="text" name="username" id="username" class="form-control <?= $validation->hasError('username') ? 'is-invalid' : ''; ?>" style="height: 38px" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.slice(1)" value="<?= ucfirst($user['username']); ?>">
						...
						<div class="input-group">
							...
							<select name="role_id" id="role_id" class="form-control <?= $validation->hasError('role_id') ? 'is-invalid' : ''; ?>" style="width: 92%;">
								...
								<?php foreach( $role as $row ) : ?>
									<option <?= $row['id_role'] == $user['role_id'] ? 'selected' : ''; ?> value="<?= $row['id_role']; ?>" <?= set_select('role_id', $row['id_role']); ?>><?= ucfirst($row['nama_role']); ?></option>
								<?php endforeach; ?>
							</select>
				...
				<div class="col">
					...
					<div class="text-center">
						<img id="previewFoto" src="<?= base_url('public'); ?>/uploads/foto/<?= $user['foto']; ?>" alt="image" class="img-fluid rounded" width="170" style="height: 150px" alt="Preview Gambar">
					</div>
						...
						<div class="col">
							<div class="form-check mt-2" style="margin-left: -20px">
								<div class="custom-control custom-radio custom-control-inline">
									<input class="form-check-input custom-control-input is_active <?= $validation->hasError('is_active') ? 'is-invalid' : ''; ?>" type="radio" name="is_active" id="aktif" value="1" <?= ($user['is_active'] == 1 ? 'checked' : ''); ?>>
									<label class="custom-control-label aktif" for="aktif"><small style="font-size: 14px">Aktif</small></label>
								</div>
								<div class="custom-control custom-radio custom-control-inline">
									<input class="form-check-input custom-control-input is_active <?= $validation->hasError('is_active') ? 'is-invalid' : ''; ?>" type="radio" name="is_active" id="tidak_aktif" value="0"  <?= ($user['is_active'] == 0 ? 'checked' : ''); ?>>
